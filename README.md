...
3.  Пакет "exeption" содержит 8 классов:
  
    
      - класс "InvalidCredentialsException" представляет собой пользовательское исключение, которое наследуется от 
    класса RuntimeException.Класс используется для сигнализации о проблемах с аутентификацией пользователя, таких 
    как неправильный логин или пароль. Это позволяет более точно обрабатывать ошибки, связанные с аутентификацией, 
    и предоставлять пользователям соответствующие сообщения об ошибках.
    
    
      - класс "UnauthorizedAccessException" пользовательское исключение используемое для обработки ошибок, связанных
    с несанкционированным доступом к ресурсам или операциям в приложении, это позволяет более точно управлять логикой 
    обработки ошибок.Конструктор с двумя параметрами позволяет передавать как сообщение об ошибке, так и сообщать 
    ...
    строковый параметр message,  который представляет собой сообщение об ошибке.
      ...
      - класс "FileUploadException" является пользовательским исключением, которое наследует от RuntimeException. 
    Этот класс предназначен для обработки ошибок, связанных с загрузкой файлов. Класс может быть использован для 
    обозначения ситуации, когда загрузка файла не удалась по какой-либо причине, такой как проблемы с доступом к 
    файлу, превышение размера файла или ошибки ввода-вывода.


      - класс "NoFilesFoundException" является пользовательским исключением, которое наследует от RuntimeException.
    Класс предназначен для обработки ошибок, связанных с отсутствием файлов в системе, например, когда пользователь 
    запрашивает список файлов, но в базе данных или файловой системе нет соответствующих записей.


      - класс "FileDeletionException" является пользовательским исключением, которое наследует от "RuntimeException". 
    Этот класс предназначен для обработки ошибок, связанных с удалением файлов. Класс позволяет четко идентифицировать 
    ошибки, возникающие в процессе удаления файлов.


      - класс "FileDownloadException" является пользовательским исключением, которое наследует от "RuntimeException". 
    Этот класс предназначен для обработки ошибок, связанных со скачиванием файлов.Использование этого исключения 
    позволяет обеспечивать пользователю более понятные сообщения об ошибках.
    ...
    централизованно.Класс использует Logger для записи предупреждений и ошибок. В классе прописаны следующие методы:
              ...
              ответ с кодом состояния 401 и сообщением о неверных учетных данных.
                ...
                метод "handleFileNotFound" для обработки исключений FileNotFoundException, возвращает ответ со 
              статусом 404 и сообщением о том, что файл не найден.
              
                метод "handleFileAlreadyExists" для обработки случая когда файл уже существует, возвращает 
              сообщение со статусом 409 и сообщение что файл уже существует. 
                
                метод "handleFileDeletionException" для обработки случая когда произошла ошибка удаления файла, 
              возвращает ответ со статусом  500 и сообщение об ошибке удаления файла.

                метод "handleFileUploadException" для обработки случая когда произошла ошибка загрузки файла, 
              возвращает статус ошибки 500 и сообщение  об ошибке загрузке файла.

                метод "handleFileDownloadException" для обработки случая, когда произошла ошибка скачивания файла, 
              возвращает статус ошибки 500 и сообщение об ошибке скачивания файла.

                метод "handleUnauthorizedAccess" для обработки случаев несанкционированного доступа, возвращается 
              статус ошибки 401 и сообщение об ошибке.

                метод "handleNoFilesFound" для обработки случаев когда список файлов пуст, возвращается статус 
              ошибки  404 и сообщение об отсутствии файлов .
...
6.  пакет "service" содержит 2 класса:
    ...
    информации, предупреждений и ошибок в процессе работы сервиса.
                ...
                проблема при чтении выбрасывается сообщение об ошибке. Метод помечен аннотацией "@Transactional" - 
                при возникновении ошибки во время работы метода все изменения этой транзакции будут отменены и 
                данные пользователя откатятся на момент начала выполнения операции данным методом.
